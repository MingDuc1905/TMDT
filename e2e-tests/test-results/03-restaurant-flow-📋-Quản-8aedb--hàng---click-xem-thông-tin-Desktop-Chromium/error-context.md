...
- Name: 03-restaurant-flow.spec.ts >> 📋 Quản lý đơn hàng (Order List) >> [TC-3.6] Chi tiết đơn hàng - click xem thông tin
- Location: tests\03-restaurant-flow.spec.ts:118:7
...
```ts
  ...
  32  |   // Solution: goto trực tiếp /Restaurant, retry nhanh với domcontentloaded
  33  |   // ponytail: cold start → goto /Restaurant với timeout vừa đủ, 2 retries
  ...
  37  |       try {
  38  |         await page.goto('/Restaurant', { waitUntil: 'domcontentloaded', timeout: 20_000 });
  39  |         if (page.url().includes('/Restaurant')) break;
  40  |       } catch {
  41  |         console.log(`⚠️ Fallback goto Restaurant #${retry+1} failed`);
